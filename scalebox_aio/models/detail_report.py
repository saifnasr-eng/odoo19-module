...
class ScaleboxDetailReport(models.TransientModel):
    ...
    def _xlsx_title(self):
        return _('Detailed Sales Report') if self.report_type == 'sale' \
            else _('Detailed Purchases Report')

    def _xlsx_filename(self):
        return 'detail_%s_%s.xlsx' % (self.report_type, self.date_to)

    def _xlsx_sheet_name(self):
        return _('Detailed Report')

    def _xlsx_meta(self):
        meta = [(_('From'), str(self.date_from)), (_('To'), str(self.date_to))]
        if self.partner_id:
            meta.append((_('Customer / Vendor'), self.partner_id.name))
        meta += [(_('Total'), self.gross_total),
                 (_('Total Returns'), self.returns_total),
                 (_('Net'), self.net_total)]
        return meta

    def _xlsx_columns(self):
        return [
            (_('Date'), 'date', 'text'),
            (_('Reference'), 'name', 'text'),
            (_('Customer / Vendor'), 'partner', 'text'),
            (_('Type'), 'kind', 'text'),
            (_('Untaxed'), 'amount_untaxed', 'money'),
            (_('Tax'), 'amount_tax', 'money'),
            (_('Total'), 'amount_total', 'money'),
        ]

    def _xlsx_rows(self):
        kinds = dict(self.line_ids._fields['doc_kind'].selection)
        rows = []
