...
class ScaleboxEasyPurchase(models.Model):
    """Easy Purchase screen: creates the purchase order, receives stock, posts the vendor bill,
    and pays it in one step, fully using the standard Odoo engine."""

    _name = 'scalebox.easy.purchase'
    _description = 'Scalebox - Easy Purchase'
    ...
    vendor_ref = fields.Char(string='Vendor Bill Reference')
    line_ids = fields.One2many(
        'scalebox.easy.purchase.line', 'easy_purchase_id', string='Lines',
        copy=True)
    ...
    pay_now = fields.Boolean(
        string='Instant Payment', default=False,
        help='On confirmation, a payment for the full bill amount is registered automatically.')
        ...
        for vals in vals_list:
            if not vals.get('name') or vals['name'] == _('New'):
                vals['name'] = self.env['ir.sequence'].next_by_code(
                    'scalebox.easy.purchase') or _('New')
        return super().create(vals_list)

    def unlink(self):
        if any(rec.state == 'done' for rec in self):
            raise UserError(_('Cannot delete a confirmed purchase. Cancel it first.'))
        return super().unlink()
    ...
    def action_confirm(self):
        """Runs the full cycle: purchase order -> receipt -> vendor bill -> payment."""
        for rec in self:
            if rec.state != 'draft':
                continue
            if not rec.line_ids:
            ...
            if rec.pay_now and not rec.payment_journal_id:
                raise UserError(_('Select a payment method (cash or bank) or disable instant payment.'))

            # 1) Purchase order
            order = self.env['purchase.order'].create(rec._prepare_purchase_order_vals())
            order.button_confirm()
            rec.purchase_order_id = order
            ...
            if not invoice:
                raise UserError(_('Could not create the vendor bill. Check product settings.'))
            invoice.invoice_date = rec.date
            if rec.vendor_ref:
                invoice.ref = rec.vendor_ref
            invoice.action_post()
            ...
            rec.message_post(body=_(
                'Operation confirmed: Purchase order %s | Bill %s',
        ...
        for rec in self:
            if rec.invoice_id and rec.invoice_id.state == 'posted':
                if rec.invoice_id.payment_state in ('paid', 'in_payment', 'partial'):
                    raise UserError(_(
                        'Vendor bill %s is fully or partially paid. Cancel the payment from '
                        'the Accounting screen first.', rec.invoice_id.name))
                rec.invoice_id.button_draft()
                rec.invoice_id.button_cancel()
            if rec.purchase_order_id and rec.purchase_order_id.state != 'cancel':
    ...
    # Helpers
    # ------------------------------------------------------------------
    def _po_line_tax_field(self):
        """Odoo 18 uses 'taxes_id'; Odoo 19 renamed it to 'tax_ids'."""
        return 'tax_ids' if 'tax_ids' in self.env['purchase.order.line']._fields else 'taxes_id'
    ...
    def _prepare_po_line_vals(self, line):
        ...
        vals[self._po_line_tax_field()] = [(6, 0, line.tax_ids.ids)]
        return vals
    ...
    def _prepare_purchase_order_vals(self):
        ...
        return {
            ...
            'order_line': [(0, 0, self._prepare_po_line_vals(line))
                           for line in self.line_ids],
        }


class ScaleboxEasyPurchaseLine(models.Model):
    _name = 'scalebox.easy.purchase.line'
    _description = 'Scalebox - Easy Purchase Line'

    easy_purchase_id = fields.Many2one(
        'scalebox.easy.purchase', string='Purchase Operation', required=True,
        ondelete='cascade', index=True)
    company_id = fields.Many2one(
        related='easy_purchase_id.company_id', store=True)
    currency_id = fields.Many2one(
        related='easy_purchase_id.currency_id')
    ...
    @api.depends('product_id', 'easy_purchase_id.warehouse_id')
    def _compute_qty_available(self):
        for line in self:
            if line.product_id and line.product_id.is_storable:
                wh = line.easy_purchase_id.warehouse_id
        ...
        for line in self:
            ...
            if line.tax_ids:
                taxes = line.tax_ids.compute_all(
                    ...
                    partner=line.easy_purchase_id.partner_id,
                )
